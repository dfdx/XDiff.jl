...
# @scalardiff *(x::Number, y::AbstractArray) 1 sum(y)
# @scalardiff *(x::AbstractArray, y::Number) 1 y
# @scalardiff *(x::AbstractArray, y::AbstractArray) 1 y'

@scalardiff *(x::Number, y::Number) 2 x
# @scalardiff *(x::Number, y::AbstractArray) 2 x
# @scalardiff *(x::AbstractArray, y::Number) 2 sum(x)
# @scalardiff *(x::AbstractArray, y::AbstractArray) 2 x'
...
# @scalardiff (x::Number ^ 2::Number) 1 (2x)

@scalardiff (x::Number / y::Number) 1 (x / y)
# @scalardiff (x::AbstractArray / y::Number) 1 x ./ y
@scalardiff (n::Number / x::Real) 2 (-n * x .^ -2.0)
# @scalardiff (x::AbstractArray / y::Real) 2 (sum(-x .* y) / (y * y))
@scalardiff (x::Number ./ y::Number) 1 (x / y)
# @scalardiff (x::AbstractArray ./ y::Number) 1 x ./ y
@scalardiff (n::Number ./ x::Real) 2 (-n * x .^ -2.0)
# @scalardiff (x::AbstractArray ./ y::Real) 2 (sum(-x .* y) / (y * y))
...
# @scalardiff sum(x::AbstractArray) 1 ones(size(x))

# dot product

# @scalardiff dot(x::Number, y::Number) 1 y
# @scalardiff dot(x::Number, y::Number) 2 x

# @scalardiff vecdot(x::AbstractVector, y::AbstractVector) 1 y
# @scalardiff vecdot(x::AbstractVector, y::AbstractVector) 2 x

# @scalardiff dot(x::AbstractArray, y::AbstractArray) 1 y
# @scalardiff dot(x::AbstractArray, y::AbstractArray) 2 x

# trigonomeric functions

@scalardiff sin(x::Number) 1 cos(x)
# @scalardiff sin(x::AbstractArray) 1 cos(x)
@scalardiff cos(x::Number) 1 -sin(x)
# @scalardiff cos(x::AbstractArray) 1 -sin(x)

@scalardiff tan(x::Number) 1 (1. + tan(x)  * tan(x))
# @scalardiff tan(x::AbstractArray) 1 (1. + tan(x) .* tan(x))

@scalardiff sinh(x::Number) 1 cosh(x)
# @scalardiff sinh(x::AbstractArray) 1 cosh(x)
...
# @scalardiff exp(x::AbstractArray) 1 exp(x)
...
# @scalardiff abs(x::AbstractArray) 1 (sign(x) .* x)
...
# @scalardiff transpose(x::AbstractArray) 1 transpose(ones(size(x)))

@scalardiff size(x::Any) 1 0.
@scalardiff size(x::Any, y::Any) 1 0.
@scalardiff size(x::Any, y::Any) 2 0.
